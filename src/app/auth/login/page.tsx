...
import { authenticateMerchant } from "@/lib/actions";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const res = await authenticateMerchant(phone, pin);
    if (res?.error) {
      setError(res.error);
      setLoading(false);
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
        <h1 className="text-2xl font-black text-slate-900 mb-2 text-center">Welcome Back! 👋</h1>
        <p className="text-sm text-slate-500 mb-8 text-center">Log in to manage your WhatsApp Store</p>
        ...
        <div className="mt-6 text-center text-sm text-slate-500 font-medium">
          Don't have a store? <a href="/auth/signup" className="text-indigo-600 font-bold hover:underline">Create one</a>
        </div>
      </div>
    </div>
  );
